...
app.post('/api/admin/create-license', (req, res) => {
    const { email, days, customKey } = req.body;
    if (!email) return res.status(400).json({ error: 'User email is required' });
    
    const db = loadLicenses();
    const licenseKey = customKey ? customKey.trim().toUpperCase() : ('SWIFT-' + Math.random().toString(36).substring(2, 6).toUpperCase() + '-' + Math.random().toString(36).substring(2, 6).toUpperCase());
